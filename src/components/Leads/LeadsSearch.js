...
const LeadsSearch = () => {
  ...
  const querySearch = "SELECT c.id AS contact_id,  c.full_name,  c.email,  c.sales_rep, c.phone, c.created_date, CONCAT('$', FORMAT(COALESCE(SUM(p.payment_amount), NULL), 2)) AS sales,  CASE WHEN MIN(c.utm_source) <> 'null' THEN MIN(c.utm_source) END AS utm_source,  CASE WHEN MAX(c.utm_source) <> 'null' THEN MAX(c.utm_source) ELSE CASE WHEN MIN(c.utm_source) <> 'null' THEN MIN(c.utm_source) END END AS lastSource FROM  Contacts c LEFT JOIN  Payments p ON c.id = p.fk_contact WHERE c.created_date BETWEEN '" + formattedDateStart.toString() + "' AND '" + formattedDateEnd.toString() + "'     AND (c.full_name LIKE '%" + searchTerm + "%' OR c.email LIKE '%" + searchTerm + "%' OR c.phone LIKE '%" + searchTerm + "%') GROUP BY  c.email ORDER BY c.created_date DESC"
